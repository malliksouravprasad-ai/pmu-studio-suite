...
from engine import init_state, reset_state, get_workspace, get_job, set_job, set_raw_df, has_data, DashboardStudioJob
...
with c2:
    ...
    dashboard_title = st.text_input("Dashboard Title", value=job.dashboard_job.dashboard_title)

if uploaded is None:
    if has_data():
        st.info(f"**{job.source_filename}** is loaded — navigate to **KPIs** or **Charts** to continue.")
    else:
...
st.success("File loaded. Navigate to **KPIs** to continue.")
