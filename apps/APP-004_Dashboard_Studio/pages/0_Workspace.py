"""Page 0 — Workspace selection for Dashboard Studio."""
import sys
import os
...
st.set_page_config(page_title="Workspace — Dashboard Studio", page_icon="📁", layout="wide")
init_state()
...
_user_tag = _col_tag.text_input("Your Name / User Tag",
    ...
    placeholder="e.g. Malli, District_A, Team_FLN", key="ws_tag_004")
st.session_state["pmu_user_tag"] = _user_tag.strip()
if _col_clr.button("Clear", key="ws_clr_004", use_container_width=True):
...
_show_all = st.toggle("Show all users' workspaces", value=False, key="ws_all_004")
...
for ws_meta in _ws_list:
    ...
    with st.container(border=True):
        ...
        if sum(len(v) for v in configs.values()):
            c1.caption(f"💾 {sum(len(v) for v in configs.values())} layout(s) saved")
        with c2:
